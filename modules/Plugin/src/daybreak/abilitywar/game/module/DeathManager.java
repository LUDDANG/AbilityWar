 ...
 * @author Daybreak 새벽
 */
@ModuleBase(DeathManager.class)
public class DeathManager implements ListenerModule {

	private final Game game;
	private boolean abilityReveal = DeathSettings.getAbilityReveal(), autoRespawn = DeathSettings.getAutoRespawn(), abilityRemoval = true;
	...
	public final void onPlayerDeath(final PlayerDeathEvent e) {
		...
		if (game.isParticipating(dead)) {
			...
			if (!event.isCancelled()) {
				...
				if (operation.getAbilityRemoval() && abilityRemoval) victim.removeAbility();
